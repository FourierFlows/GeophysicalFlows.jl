...
"""
function Problem_nf(nlayers::Int,                             # number of fluid layers
                          dev = CPU();
  ...
  params = Params_nf(nlayers, g, f₀, β, ρ, H, U, eta, topographic_pv_gradient, μ, ν, nν, grid; calcFq)
...
$(TYPEDFIELDS)
"""
struct Params_nf{T, Aphys3D, Aphys2D, Aphys1D, Atrans4D, Trfft} <: AbstractParams
  # prescribed params
...
function Params_nf(nlayers, g, f₀, β, ρ, H, U, eta, topographic_pv_gradient, μ, ν, nν, grid; calcFq=nothingfunction, effort=FFTW.MEASURE)
  dev = grid.device
  T = eltype(grid)
  A = device_array(dev)
  ...
  if nlayers==1
    ...
    if nlayers==2
      ...
      return Params_nf(nlayers, T(g), T(f₀), T(β), A(ρ), A(H), U, eta, topographic_pv_gradient, T(μ), T(ν), nν, calcFq, A(g′), Qx, Qy, S, S⁻¹, rfftplanlayered)
    end
  end
end
